...
void video_refresh_timer(void *userdata)
{
	...
	if (video->stream_index >= 0)
	{
		...
		else
		{
            ...
            if (NULL == video->sws_ctx){
                video->sws_ctx = sws_getContext(video->video_ctx->width, video->video_ctx->height, video->video_ctx->pix_fmt,
                                                video->displayFrame->width,video->displayFrame->height,
                                                (AVPixelFormat)video->displayFrame->format, SWS_BILINEAR, NULL, NULL, NULL);
            }
